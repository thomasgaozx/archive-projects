...
{
    /// <summary>
    /// If caught, shall suggest "help" menu
    /// </summary>
    public class BadCommandException : Exception
    {
        public BadCommandException() : base("Bad command!")
        {
        }

        public BadCommandException(string badCommand)
        {
        }

        public BadCommandException(IEnumerable<string> commands) : base($"The command '{commands.Aggregate((s1, s2) => s1 + " " + s2)}' is not recognized")
        {
